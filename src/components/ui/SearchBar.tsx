...
export default function SearchBar({ placeholder, locale, initialValue = '' }: { placeholder: string, locale: string, initialValue?: string }) {
  ...
  return (
    ...
    >
      <input 
        className="flex-1 bg-transparent border-none focus:ring-0 px-6 py-4 text-on-surface placeholder:text-on-surface-variant/70 text-lg w-full outline-none" 
      ...
      <button 
        type="submit"
        className="px-6 py-4 bg-primary text-white flex items-center justify-center transition-colors hover:bg-primary/90 focus:outline-none focus:ring-2 focus:ring-primary/50 focus:ring-offset-2"
      >
