...
export default async function BotsPage({ 
  ...
  const selectedTypes = Array.isArray(resolvedSearchParams.types) ? resolvedSearchParams.types : 
                        typeof resolvedSearchParams.types === 'string' ? [resolvedSearchParams.types] : ['bot'];
        ...
        <div className="flex-1 space-y-8">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-surface-container-lowest p-5 rounded-xl border border-outline-variant/15 shadow-sm">
            <nav className="flex items-center gap-2 text-sm font-medium">
              ...
              <span className="text-slate-900 font-black dark:text-white uppercase tracking-tight font-['Inter','Tajawal']">{dict.breadcrumbBots}</span>
            </nav>
          ...
          <Pagination currentPage={page} totalPages={totalPages || 1} baseUrl={`${prefix}/bots`} dict={dict} />
        </div>
      </main>
    </>
  );
