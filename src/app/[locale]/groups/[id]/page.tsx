      ...
      <main className="min-h-screen pt-12">
        ...
        <section className="relative w-full max-w-[1536px] mx-auto px-6 pt-2 pb-12">
          ...
          <nav className="flex items-center gap-2 text-sm font-medium text-slate-500 dark:text-slate-400 mb-8 w-fit px-5 py-2 rounded-2xl border border-slate-200/50 dark:border-slate-800/50 bg-surface-container-lowest shadow-sm font-['Inter','Tajawal']">
            ...
            <Link href={`${prefix}/groups`} className="hover:text-primary transition-colors">{dict.breadcrumbGroups}</Link>
            <span className="material-symbols-outlined text-xs rtl:rotate-180">chevron_right</span>
          ...
          <div className="bg-gradient-to-br from-sky-500/5 via-surface-container-lowest to-surface-container rounded-2xl p-8 md:p-14 border border-slate-200/50 dark:border-slate-700/30 flex flex-col lg:flex-row gap-8 lg:gap-14 items-start lg:items-center relative overflow-hidden shadow-xl shadow-slate-200/20 dark:shadow-none">
            
            <div className="absolute top-0 right-0 w-[600px] h-[600px] bg-sky-500/10 rounded-full blur-[120px] -translate-y-1/2 translate-x-1/3 pointer-events-none"></div>
            ...
            <div className="flex-1 space-y-5 z-10">
              ...
              <div className="flex flex-wrap items-center gap-5 pt-6">
                <Link href={item.link || '#'} target="_blank" className="relative overflow-hidden group px-10 py-5 bg-primary hover:bg-sky-600 active:bg-sky-700 text-white font-black text-xl rounded-lg shadow-2xl shadow-primary/30 transition-all duration-300 active:scale-95 flex items-center gap-3 font-['Inter','Tajawal']">
                  <span className="material-symbols-outlined relative z-10 text-2xl" style={{fontVariationSettings: '"FILL" 1'}}>chat</span>
                  <span className="relative z-10 tracking-tight">{dict.detailsJoin}</span>
                </Link>
                
                <button className="w-16 h-16 rounded-lg bg-white dark:bg-slate-800 hover:bg-slate-50 border border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-300 transition-all duration-300 flex items-center justify-center group active:scale-95 shadow-lg shadow-slate-200/10 dark:shadow-none">
        ...
        <section className="relative z-30 max-w-[1536px] mx-auto px-6 mb-16">
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-6">
            <div className="bg-surface-container-lowest p-8 rounded-xl border border-slate-200/60 dark:border-slate-800 shadow-sm flex flex-col items-center justify-center text-center transition-all hover:shadow-md hover:-translate-y-1">
              ...
              <span className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 font-['Inter','Tajawal']">{dict.cardMembers}</span>
            </div>
        ...
        <section className="max-w-[1536px] mx-auto px-6 grid grid-cols-1 lg:grid-cols-12 gap-10 lg:gap-14 mb-24">
          <div className="lg:col-span-8 space-y-10">
            <div className="bg-surface-container-lowest rounded-xl p-8 md:p-12 shadow-sm border border-slate-200/60 dark:border-slate-800 space-y-8">
              <h2 className="text-2xl md:text-3xl font-black text-slate-900 dark:text-white border-s-8 border-primary ps-6 font-['Inter','Tajawal']">
                {locale === 'ar' ? 'حول هذه المجموعة' : 'About this Group'}
              </h2>
              <div className="prose prose-slate dark:prose-invert max-w-none text-lg leading-relaxed font-['Inter','Tajawal']">
          ...
          <aside className="lg:col-span-4 space-y-10">
            <div className="p-10 bg-gradient-to-br from-surface-container to-surface-container-high rounded-2xl border border-slate-200/60 dark:border-slate-800 text-center relative overflow-hidden group shadow-lg">
              ...
              <h4 className="font-black text-slate-800 dark:text-white text-xl mb-3 font-['Inter','Tajawal']">{dict.promoteGroup}</h4>
